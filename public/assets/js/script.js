$(document).ready(function () {
    ...
    $('#form-select-filter').change(function(e){
        var choose = $(this).val();
        var token = $("input[name='_token']").val();
        var getStatusDomain = $(this).attr('attr_url')
        $.ajax({
            url: "/sort",
            type: 'GET',
            data: {
            ...
            const defaultDomainLocalHost = '127.0.0.1:8000'
            var imgSrc = `https://${getStatusDomain}/storage/images/products/`;
            if(getStatusDomain == defaultDomainLocalHost)
            {
                var imgSrc = `http://${getStatusDomain}/storage/images/products/`;        
            }
            var html = '';
